...
This is CGLib v1.0 build 23, the first ever official version with common sources for both Windows and Mac OS X.
...
On both systems, CGLib is compiled into a static library in Debug mode.



File "Makefile" on directory "cglib.dev" may be adapted to other operating systems (e.g., Linux, CygWin, etc.)
...
This package contains a top level directory, cglib.dev, with the following:
     ...
     - File config.ini, the configuration file requied by all applications built with CGLib
...
On the Mac, applications can be built either with XCode or with the make utility.
...
Your application source and include files must be place in the "cglib.dev/template" directory and assigned
...
To build, open a Terminal, change to the "cglib.dev" directory and type "make".
